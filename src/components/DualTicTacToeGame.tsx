...
const DualTicTacToeGame = ({ playerName, difficulty, onDifficultyChange, onNameChange, onModeChange }: DualTicTacToeGameProps) => {
  ...
  const activePlayerGame = game1.currentPlayer === 'X' ? game1 : game2;
  ...
  useEffect(() => {
    const handleKeyPress = (event: KeyboardEvent) => {
      ...
      if (event.key === ' ' && !winner) {
        ...
        if (game1.currentPlayer === 'X') makeMove(1, sharedSelectedPosition);
        else if (game2.currentPlayer === 'X') makeMove(2, sharedSelectedPosition);
      }
    };
    window.addEventListener('keydown', handleKeyPress);
    return () => window.removeEventListener('keydown', handleKeyPress);
  }, [sharedSelectedPosition, updateSelectedPosition, makeMove, game1.currentPlayer, game2.currentPlayer, winner]);

  const getCellClass = (cell: 'X' | 'O' | null, index: number, isPlayerTurn: boolean) => {
    ...
    const turnClass = isPlayerTurn ? "cursor-pointer hover:bg-gray-700/50 hover:scale-105 hover:shadow-lg hover:shadow-blue-500/20" : "cursor-not-allowed";
  ...
  const getTimeBarColor = () => {
    if (activePlayerGame.timeLeft > 3) return 'bg-gradient-to-r from-green-500 to-blue-500';
    if (activePlayerGame.timeLeft > 1) return 'bg-gradient-to-r from-yellow-500 to-orange-500';
    ...
    <div className="w-full max-w-4xl mx-auto">
      <Card className="bg-gray-900/80 backdrop-blur-lg border-gray-700/50 shadow-2xl">
        <CardHeader className="text-center space-y-4">
          ...
          {activePlayerGame.isGameActive && !winner && (
            <div className="space-y-2">
              <div className="flex items-center justify-center gap-2 text-white/80">
                <Clock className="w-4 h-4" />
                <span className="text-sm font-medium">Tempo: {activePlayerGame.timeLeft.toFixed(1)}s</span>
              </div>
              <div className="w-full bg-gray-700 rounded-full h-2">
                <div className={`h-2 rounded-full transition-all duration-100 ${getTimeBarColor()}`}
                  style={{ width: `${(activePlayerGame.timeLeft / difficultySettings[difficulty].time) * 100}%` }}/>
              </div>
            </div>
          )}
        ...
        <CardContent className="space-y-6">
          <div className="flex flex-col md:flex-row gap-8 justify-center items-center">
            {games.map((game, boardNum) => (
              <div key={boardNum} className="flex flex-col items-center gap-2">
                <div className="text-lg font-semibold">
                  <span className={game.currentPlayer === 'X' ? 'text-blue-400' : 'text-cyan-400'}>
                    {game.currentPlayer === 'X' ? `Sua Vez` : 'Vez do Computador'}
                  </span>
                </div>
                <div className="grid grid-cols-3 gap-2 w-fit">
                  {game.board.map((cell, index) => (
                    <div
                      key={index}
                      className={getCellClass(cell, index, game.currentPlayer === 'X')}
